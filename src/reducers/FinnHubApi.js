...
export const FinnHubApi = createApi({
    ...
    endpoints:(builder) =>({
        getAllNews: builder.query({
            query: () => `news?category=general&token=${import.meta.env.VITE_FINNHUB_API}`
          }),
        ...
        getExchangeNews: builder.query({
            query: () =>  `news?category=exchange&token=${import.meta.env.VITE_FINNHUB_API}`
        }),
        getGoldNews: builder.query({
            query:() =>  `news?category=gold&token=${import.meta.env.VITE_FINNHUB_API}`
        }),     
    })
})

export const { useGetAllNewsQuery , useGetCurrenyNewsQuery , useGetCyrptoNewsQuery, useGetEmtiaNewsQuery, useGetExchangeNewsQuery, useGetGoldNewsQuery } = FinnHubApi